...
Page({
  ...
  formSubmit(e) {
    ...
    var param = {
      ...
      thumb: vm.data.pics_url.join(','),//图片
  ...
  init(){
    wx.request({
      ...
      success:ret=>{
        ...
        if (ret.data.status == 1) {
          var array=[];
          for (var i = 0; i < ret.data.data.angling_site_type_list.length;i++){
            array.push(ret.data.data.angling_site_type_list[i].name)
          }
         vm.setData({
           list:ret.data.data.angling_site_type_list,
           array
         })
        } else {
          wx.showToast({
